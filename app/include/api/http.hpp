...
public:
    ...
    std::string _request(const std::string& method, const std::string& url, const std::string& data);
    ...
    // Post methods
    template <typename... Ts>
    static std::string request(const std::string& method, const std::string& url, const std::string& data, Ts&&... ts) {
        HTTP s;
        set_option(s, std::forward<Ts>(ts)...);
        return s._request(method, url, data);
    }
